<html>
<head>
	<title>Index Page</title>
	<link rel="stylesheet" href="../assets/css/home.css" />
</head>
<body>
<center>
	<table width="800px" border="1" align="center" cellspacing="0">
		<tr width="800px">
			<td class="header_top" width="100px"><img src="../assets/f5.jpg" width="130px" height="130px"></td>
			
			<td class ="menubar" colspan="2" align="center">
			  <h2> <b>Sunflower Govt. High School</b></h2>
				<a href="../home.php">HOME</a> | 
				<a href="registration.php">REGISTRATION</a> | 
				<a href="login.php">LOGIN</a> | 
				<a href="">CONTACT</a>
			</td>
		</tr>
		<tr height="400px">
			<td width="100" id="center_content">
				
			</td>
			<td colspan="2">
		   <table border="0" align="center" class="contact">
		    <tr >
			<td style="color:#663300 ;font-style:italic;">
			
	        <h1>Address:</h1>No. 51, road no.-23,xyz, Dhaka-1212

            <h1>Phone:</h1>+880 1734103280

             <h1>Email:</h1>sunflowerschool@gmail.com

             <h1>Website:</h1>https://www.sunflowerschool.edu/

	         
	        </td>
	        </tr>
	
            </table>


			</td>
			
		</tr>
		<tr height="100px">
			<td id="article" colspan="3" align="center" >
				
			“Give me an educated mother, I shall promise you the birth of a civilized, educated nation.”<br>
								— Napoleon
				
			</td>
		</tr>
	</table>
	</center>
</body>
</html>








